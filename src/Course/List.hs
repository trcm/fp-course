...
map ::
  ...
  -> List a
  -> List b
-- map f items = foldRight (\item acc -> ((f item) :. acc)) Nil items
map _ Nil = Nil
...
filter f list =
  let
    test item acc = case (f item) of
             True -> (item :. acc)
             False -> acc
  in
    foldRight test Nil list
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 s = error "todo:lengthGT4"
