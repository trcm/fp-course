...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok f s p =
  between (charTok f) (charTok s) p
  -- error "betwwen char tok"
...
hex =
  -- error "todo"
  let
    h s = case readHex s of
          Empty -> 0
          Full n -> n
  in
    do
      a <- replicateA 4 (satisfy isHexDigit)
      pure ((chr . h) a)
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma f s p=
  sepby (betweenCharTok f s p) (charTok ',')
