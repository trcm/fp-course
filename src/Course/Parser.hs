...
(|||) ::
  ...
  -> Parser a
(|||) pa pb =
  P (\i -> let pr1 = parse pa i
           in if isErrorResult pr1 then parse pb i else pr1)
...
satisfy p =
  character >>= 
  lift3 bool unexpectedCharParser pure p 

  -- \c -> bool (unexpectedCharParser c) (pure c) (p c)
